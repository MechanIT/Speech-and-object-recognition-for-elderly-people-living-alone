...
def recognize_speech_from_mic(recognizer, microphone, timeout=10):
    ...
    with microphone as source:
        ...
        listen_thread = threading.Thread(target=listen_in_background, args=(recognizer, source, audio_queue))
        listen_thread.start()
        listen_thread.join(timeout)

        if listen_thread.is_alive():
            print("음성 인식 시간 초과")
            return {"success": False, "error": "음성 인식 시간 초과", "transcription": None}
        
        if not audio_queue.empty():
            audio = audio_queue.get()
        else:
            return {"success": False, "error": "음성을 이해할 수 없음", "transcription": None}
...
def start_streamlit_app():
    ...
    os.system("streamlit run webcam_page.py")
...
if __name__ == "__main__":
    ...
    if response["success"]:
        ...
        if response["error"] == "음성 인식 시간 초과":
            play_text('시간 초과. 영상 탐지를 시작합니다.')
            start_streamlit_app()
